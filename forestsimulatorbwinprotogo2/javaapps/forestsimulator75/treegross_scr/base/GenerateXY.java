...
/** calculates xy values for trees in the area defined by cornerpoints,
 *   there is a minimum distance of two trees . This 
 *   distance becomes smaller with each try, max. 20 tries  
 *   This method can be only used if
 *       - all trees have weight factor (fac) = 1.0
 */
...
// determin xmin, xmax, ymin and ymax of area defined by corners
	  xmin=99999.9; ymin=xmin; xmax=-xmin; ymax=xmax; 
	  int i,j,ic;
                 ...
                 if (skidtrail == false || skidtrailDistance==0.0){ 
                   if ( groupRadius == 0.0 || groupCoverArea == 0.0){  
                          ...
                          if (groupRadius > 0.0) {
                              ...
                              if (groupCoverArea > 0.85) groupCoverArea = 0.0;
                          }
                               
                   }  
                   else{  // Group position
                          xg=oldpositionX-groupRadius+zz.random()*(2.0*groupRadius);
                          yg=oldpositionY-groupRadius+zz.random()*(2.0*groupRadius);
                          groupCoverArea = getCoverArea(st);;
                          if (groupCoverArea > 0.85) groupCoverArea = 0.0;
                   }
                 }
                     ...
                     if ( groupRadius == 0.0 || groupCoverArea == 0.0){  
                        ...
                        xg=xmin+zz.random()*(xmax-xmin)-xkorr * skidtrailWidth;
       	                yg=ymin+zz.random()*(ymax-ymin);
                        oldpositionX = xg;
                        oldpositionY = yg;
                        if (groupRadius > 0.0) {
...
// Skidtrail korrigieren                        
                        ...
                        do {
                          ...
                          if (xg > xlow) xg = xg + skidtrailWidth;
                        } while (xlow < xmax);
...
// put trees on the field
        ...
        do {
            ...
            do {
	            ...
	            if (st.tr[i].x<=0.0 && st.tr[i].y<=0.0 && merk<0) merk=i;
...
// Check if there is another tree, no tree planted in a distance of 2*diameter    
	              ...
	              if (st.tr[i].x>0.0 && st.tr[i].y>0.0 && merk!=i) {
                          ...
                          if (e<2.0*st.tr[i].d/100.0) plant=false;
                      }
                    if (plant==true){
                        st.tr[merk].x=xx;
                        st.tr[merk].y=yy;
   ...
   private double getCoverArea(Stand st) {
     ...
     for (int k=0;k<st.ntrees;k++)
        if (st.tr[k].out < 0 && st.tr[k].x >= 0.0 && st.tr[k].y >0.0){
           ...
           if (d > 0.0) d= Math.sqrt(d);
           if (d <= groupRadius) co= co+
           Math.PI*Math.pow((st.tr[k].cw/2.0),2.0);
        }  
