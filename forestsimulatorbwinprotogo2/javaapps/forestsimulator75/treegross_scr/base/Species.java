...
public class Species implements Cloneable{
    ...
    public Species clone(){
        ...
        clone.d100= new Double(this.d100);
        clone.dg= new Double(this.dg);
        clone.dgout= new Double(this.dgout);
        clone.gha= new Double(this.gha);
        clone.ghaout= new Double(this.ghaout);
        clone.h100= new Double(this.h100);
        clone.h100age= new Double(this.h100age);
        clone.hbon= new Double(this.hbon);
        clone.heightcurveUsed= new String(this.heightcurveUsed);
        clone.heightcurveUsedP0= new Double(this.heightcurveUsedP0);
        clone.heightcurveUsedP1= new Double(this.heightcurveUsedP1);
        clone.heightcurveUsedP2= new Double(this.heightcurveUsedP2);
        clone.hg= new Double(this.hg);
        clone.nha= new Double(this.nha);
        clone.nhaout= new Double(this.nhaout);
        clone.percBA= new Double(this.percBA);
        clone.percCSA= new Double(this.percCSA);
        if(price!=null) clone.price= this.price.clone();
        clone.size= new Double(this.size);
        clone.spDef= this.spDef.clone();
        clone.totalPrice= new Double(this.totalPrice);
        clone.trule= this.trule.clone();
        clone.vhaout= new Double(this.vhaout);
        clone.vjkl1= new Double(this.vjkl1);
        clone.vjkl2= new Double(this.vjkl2);
        clone.vjkl3= new Double(this.vjkl3);
        clone.vol= new Double(this.vol);        
        return clone;
    }
    
    /**set all attributes of a species to zero, this should be used if a species is new*/
    void setZero() {
        h100age=0;dg=0.0;hg=0.0;d100=0.0;h100=0.0;
        //hbon=0.0;
        dgout=0.0;
        vol=0.0;gha=.0;nha=0.0;vhaout=0.0;ghaout=0.0;nhaout=0.0;
        percBA=0.0;percCSA=0.0;
    ...
    void updateSpecies(Stand st) {
        ...
        for (int i=0;i<st.ntrees;i++){
            if (st.tr[i].d>=7 && st.tr[i].out<1 && st.tr[i].code==code ){
                nha =nha+st.tr[i].fac;
                gha =gha+st.tr[i].fac*Math.PI*(st.tr[i].d/200.0)*(st.tr[i].d/200.0);
                vol =vol+st.tr[i].fac*st.tr[i].v;
            }
            if (st.tr[i].out>0 && st.tr[i].code==code ){
                volumeOfDeadwood=volumeOfDeadwood+st.tr[i].fac*st.tr[i].volumeDeadwood;
            }
        }
        ...
        for (int i=0;i<st.ntrees;i++)
            if (st.tr[i].d>=7 && st.tr[i].out==st.year && st.tr[i].code==code ){
                nhaout =nhaout+st.tr[i].fac;
                ghaout =ghaout+st.tr[i].fac*Math.PI*(st.tr[i].d/200.0)*(st.tr[i].d/200.0);
                vhaout =vhaout+st.tr[i].fac*st.tr[i].v;
            }
        double ghaStand=0.0;
        for (int i=0;i<st.ntrees;i++)
        ...
        if (nha > 0.0) dg=200*Math.sqrt(gha/(Math.PI*nha));   //Dg
        if (nhaout > 0.0) dgout=200*Math.sqrt(ghaout/(Math.PI*nhaout));   //Dgout
        ...
        if (nha>0 && n100>0){
            ...
            if (nsiH100 > 0.0) siH100=siH100/nsiH100; else siH100=0.0;
        }
       // Check, if Petterson height curve gives realistic value if not use Uniformheight
        ...
        for (int j=0;j<st.ntrees;j++) 
            if (st.tr[j].d>=7 && st.tr[j].code==code && st.tr[j].h >=1.3 && (st.tr[j].out<1 || st.tr[j].out==st.year)) ndh=ndh+1;
        ...
        if (ndh>5) {
            ...
            if (dg > 0) hg= m.hwert(spDef.heightCurve,dg);
            if (d100 > 0) h100= m.hwert(spDef.heightCurve,d100);
            ...
            if (m.hwert(spDef.heightCurve,80)>80.0) {
                pettersonFalse = true;
                //System.out.println("Petterson false");
            }
...
//            hg=ufh.height(this,dg,dk,hk,st);
            ...
            tree.sp.hg=hk;
            FunctionInterpreter fi = new FunctionInterpreter();
            if (dk > 7.0) h100=fi.getValueForTree(tree,tree.sp.spDef.uniformHeightCurveXML);
//            h100=ufh.height(this,d100,dk,hk,st);
...
//                        st.tr[j].h=ufh.height(st.tr[j].sp,st.tr[j].d,dk,hk,st); 
        ...
        if (siH100 > 0.0 ) hbon=siH100;
            ...
            atree.code = code;
            atree.sp=this;
            if (hbon <= 0.0) hbon = atree.calculateSiteIndex();
        }
    }
