...
/** shannon calculates the Shannon-Index for the living trees of class stand */ 
   public double shannon(Stand st)  
   	{  int i,j;  //Zaehler
	   double n;
	   int art,alter; //Arbeitsvariablen
	   double h=0.0; //Arbeitsvariablen
      for (j=0;j<st.nspecies;j++)
       {  n=0;
          for (i=0;i<st.ntrees;i++)  // number of trees for each species
            if (st.tr[i].out<0 && st.tr[i].code==st.sp[j].code) n=n+1;
          if (n>0) h=h-(n/st.ntrees)*Math.log(n/st.ntrees);
       } 
      return h;
      }   
// 
