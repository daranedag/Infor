...
public class Tree3DList {
    ...
    /** Creates a new instance of Tree3DList  */
    public Tree3DList() {       
    }
    
    /** Creates a new instance of Tree3DList with all trees and attaches all trees to the parent group */
    ...
    public void setListData(Tree[] tgtrees, int ntrees, Texture2D[] bltexture, boolean speciescolor, boolean textured,  boolean sstatus, Group parent, int[] speciestoshow, boolean showdead, StandBase3D base,  int year){
        ...
        for(int i=0; i<ntrees; i++){
            if(!outBefore5(tgtrees[i], standyear)){
                ...
                if(!showSpecies(tgtrees[i].code)) trees[indexcounter].setVisible(false);
                if(!showdead && !trees[indexcounter].userdata.living && !trees[indexcounter].userdata.standing) trees[indexcounter].setVisible(false);
                indexcounter++;
            }
        }  
    }
    ...
    private int countRelevantTrees(Tree[] tgtrees, int nalltrees){
        ...
        for(int i=0; i<nalltrees; i++){
            if(!outBefore5(tgtrees[i], standyear)) result++;
        }
        return result;
    }
    ...
    public void setSpeciesToShow(int[] speciestoshow, boolean showdead){
         sts=speciestoshow;         
         if(trees!=null) for(int i=0; i< trees.length; i++){
            if(showSpecies(trees[i].userdata.spec)){
                if(!showdead && !trees[i].userdata.living && !trees[i].userdata.standing)trees[i].setVisible(false);
                else trees[i].setVisible(true);
            }
            else trees[i].setVisible(false);
        }       
    }
