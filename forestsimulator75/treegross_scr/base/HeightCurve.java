 ...
 *  There are 6 different curve types, see
...
// Return Height Kurve value, type= type of height kurve	
	public double hwert(int typ, double dk)
     { 
       double hk=0.0;
       if (dk > 0.0)
       { if (typ==0) {hk=ans[1]+(bc[1]+bc[2]*dk)*dk;}  //Parabel
         if (typ==1) {hk=1.3+dk*dk/(bc[2]+(bc[1]+ans[1]*dk)*dk);} //Prodan type
         if (typ==2) {hk=1.3+Math.exp(Math.log(1.0/(ans[1]+bc[1]*(1.0/dk)))*3.0);} //Petterson 3.0 
         if (typ==3) {hk=Math.exp(ans[1]+bc[1]*Math.log(dk)+bc[2]*Math.log(dk)*Math.log(dk));} // Korsun type
         if (typ==4) {hk=ans[1]+bc[1]*Math.log(dk);} // half logarithmic
         if (typ==5) {hk=Math.exp(ans[1]+bc[1]*Math.log(dk)+bc[2]*dk);} //Freese type
       }
       if (hk<1.3) { hk=1.3;}
       return hk;
     }  
// Return Height Kurve value, type= type of height kurve	
	public double getHeight(int typ, double dk, double b0, double b1, double b2)
     { 
       double hk=0.0;
       if (dk > 0.0)
       { if (typ==0) {hk=b0+(b1+b2*dk)*dk;}  //Parabel
         if (typ==1) {hk=1.3+dk*dk/(b2+(b1+b0*dk)*dk);} //Prodan type
         if (typ==2) {hk=1.3+Math.exp(Math.log(1.0/(b0+b1*(1.0/dk)))*3.0);} //Petterson 3.0 
         if (typ==3) {hk=Math.exp(b0+b1*Math.log(dk)+b2*Math.log(dk)*Math.log(dk));} // Korsun type
         if (typ==4) {hk=b0+b1*Math.log(dk);} // half logarithmic
         if (typ==5) {hk=Math.exp(b0+b1*Math.log(dk)+b2*dk);} //Freese type
       }
       if (hk<1.3) { hk=1.3;}
       return hk;
     }  
	...
	void corre()
	{
		...
		{	for (k=j; k<=m ; k++)
			{	jk=j+(k*k-k)/2; 
				...
				l=m*(k-1)+1; 
				rx[l]=r[jk]; 
				if (std[j]*std[k] ==0.0) {	r[jk]=0.0; }
				else {	r[jk]=r[jk]/(std[j]*std[k]); }
			}
		}
		fn=Math.sqrt(fn-1.0); 
		for (j=1; j<=m; j++)
	...
	//	System.out.println(" In Order"); 
		...
		for (j=1; j<=k; j++)
		{	l2=isave[j]; 
			if (ndep-l2 < 0) l=ndep+(l2*l2-l2)/2; 
			else l=l2+(ndep*ndep-ndep)/2; 
			ry[j]=r[l]; 
			for (i=1; i<=k; i++)
			{	l1=isave[i]; 
				if (l1-l2<0) l=l1+(l2*l2-l2)/2; 
				else l=l2+(l1*l1-l1)/2; 
				mm++; 
				rx[mm]=r[l]; 
			}
		}
	...
	//	System.out.println(" In Minv"); 
		...
		{	nk=nk+n; 
			...
			for (j=1; j<=n; j++)
			{	kj=kj+n; 
				if (j-k != 0)  rx[kj]=rx[kj]/biga ; 
			}
			dmat=dmat*biga; 
			rx[kk]=1.0/biga; 
		}
		k=n; 
		if (dmat==0.0) k=0; //zum Ende
		while (k>1)
		{
			k=k-1; 
			//		if (k<=0) break ENDE;
	...
	
	{
		...
		{	rm=rm+bc[i]*ry[i]; 
			l=isave[i]; 
			if (std[l]!=0.0) bc[i]=bc[i]*(std[l1]/std[l]); 
			else bc[i]=0.0; 
			bo=bo+bc[i]*xbar[l]; 
		}
		bo=xbar[l1]-bo; 
		...
		{	l1=k*(j-1)+j; 
			l=isave[j]; 
			if ( b[l]>0.0) {	sb[j]=Math.sqrt(Math.abs((rx[l1]/b[l])*sy)) ; }
			else {	sb[j]=-999.9; }
			if (sb[j]!=0.0) {	t[j]=bc[j]/sb[j]; } else {	sb[j]=-999.9; }
			t[j]=bc[j]/sb[j]; 
		}
