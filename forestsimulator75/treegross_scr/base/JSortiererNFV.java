...
public class JSortiererNFV extends Object implements Serializable {
    /** number of avaiable functions */ int numberOfFunctions;
    /** function information */ String funInfo;
    /** function number NFV */ int funNo;
    /** diamter at breast height [cm] */ double dbh ; 
    /** mh height where dbh is measured [m] */ double mh;
    /** dTop upper diameter (optional, if > dbh) [cm] */ double dTop;
    /** mhTop height where dTop is measured [m] */ double mhTop;
    /** height total tree height [m] */ double height;
    
    /** Assortment: start at height [m] */ double asStarth;
    /** Assortment: end at height [m]*/ double asEndh;
    /** Assortment: minimum average diameter of piece [cm] */ double asMinAvD;
    /** Assortment: maximum average diameter of piece [cm]*/ double asMaxAvD;
    /** Assortment: minimum top diameter of piece [cm]*/ double asMinTopD;
    /** Assortment: maximum top diameter of piece [cm]*/ double asMaxTopD;
    /** Assortment: minimum lengths of piece [m]*/public double asMinLen;
    /** Assortment: maximum lengths of piece [m]*/public double asMaxLen;
    /** Assortment: intervall [m] */public double asIntervall;
    /** Assortment: additional Length Percent */public double asAddLenPer;
    /** Assortment: additional Length absolut [cm] */public double asAddLenAbs;
    
    /** logFound control if assortment is in log */public boolean logFound;
    /** Length of log */public double logLength;
    /** top diameter of log */public double logTopD;
    /** middle diameter of log */public double logMidD; 
    /** additional length */public double logAddLength;
    /** regional model plugIn */ String modelRegion="default";
    ...
    /** getAssortment, defines the assortment to look for in the tree 
          Parameters to pass:
     ba = species code NFV
     du_cm = diameter at breast height [cm]
     hu_m = dbh measured at height [m] (is 13 m)
     do_cm = upper diameter [cm] (optional, if < dbh)
     ho_m = dTop measured at height [m]
     hoehe_m = tree height [m] */
    ...
    public boolean getAssortment(int function, double du_cm, double hu_m, double do_cm, double ho_m, double hoehe_m,
        ...
        mhTop=ho_m;
        height=hoehe_m;
        if (dTop > dbh) {dTop=0.0;mhTop=0.0;}
...
// Sort Routine which finds log which fullfills all criteria and is as long as possible 
    boolean findLog(){
           logFound=true;
           logTopD=asMinTopD;
           double ho=0.0;
...
// if assortment minimum top diameter less than dbh than logFound = False
           if (asMinTopD > dbh || asStarth>asEndh) logFound=false;
           else
           {
...
// regular and the diameter gets always smaller if we move up the stem
               if (logTopD < asMinTopD || logTopD > asMaxTopD) logFound=false;
               if (logMidD < asMinAvD  || logMidD > asMaxAvD ) logFound=false;
               if (logLength < asMinLen || logLength > asMaxLen) logFound=false;
           }
           return logFound;
