 ...
 */
	...
	{
// Waldzustandsbericht 2009 Hessen, 
             double rate = 0.0008;
             for (int i=0;i < st.ntrees;i++)
                if (st.tr[i].out<0 && st.tr[i].d>= 7.0) {
                 rate = 0.002;
                 if (st.tr[i].code < 113) rate=0.002;
                 if (st.tr[i].code == 211) rate=0.001;
                 if (st.tr[i].code == 511) rate=0.004;
                 if (st.tr[i].code == 711) rate=0.005;
                 if (Math.random() <= rate*5){
                     st.tr[i].out=st.year;
                     st.tr[i].outtype=1;
//                     System.out.println("Risiko Baum entnommen");
                 }
             }
