 ...
 * 
 * class CropTreeSelection selects a certain amount of crop trees from stand
 *  It starts with the strongest tree and the species with the least basal area.
 *  When selecting the next crop tree there has to be a minimum distance to the next
 *  crop tree. The percentage of crop tree for the species is defines by the percBA.
 *
 *  This routine can be only used, if :
 *       - all trees have fac = 1.0
 *       - all trees have x,y coordinates
 */
...
public class CropTreeSelection {
     ...
     * trees per species, which you load first with method addCtsp() 
     * thickest trees are selected first, habitat trees can not become crop trees
     */
	...
	for (int a=0; a<st.ntrees-1; a++){
        ...
        CropTreeSpecies ctsptemp = new CropTreeSpecies();
        ...
        do {
            ...
            for (int i=0;i<st.nspecies;i++){
                ...
                if (nselectedcroptrees >= nCTSpTarget) enoughtreesofspecies=true;
                else enoughtreesofspecies=false;
                ...
                if (selectedtree==true) selectedspecies=false;
            ...
            if (selectedspecies==true) endselection=true;

        }//do
        while (endselection==false);

        //actualize n_croptrees
        for (int i=0;i<st.nspecies;i++){getNcroptrees(st,ctsp[i].code);}  
    }
    
    
    /** method select starts the selection process, it expects the number of crop
     * trees per species, which you load first with method addCtsp()
     * thickest trees are selected first  */
        ...
        do {
            ...
            for (int i=0;i<st.nspecies;i++){
                ...
                if (enoughtreesofspecies==false){
                    ...
                    do {
                        //if layer (that contains most of trees of this species) is high enough
                        if (st.ntrees>0 && layer.getAverageHeightLayer(st, layer.getLeadingLayerSpecies(st, ctsp[i].code))>=ctsp[i].min_height){                         
   ...
   public int getNcroptrees(Stand st, int spe) {
    ...
    /** get number of sum croptrees + temp croptrees 
     you get all crop trees by giving 0 for species code*/                        
   public int getNallcroptrees(Stand st, int spe) {
    ...
    public void resetAllCropTrees(Stand st) {
        for (int i=0;i<st.ntrees;i++) {st.tr[i].crop=false; st.tr[i].tempcrop=false;} 
     }
